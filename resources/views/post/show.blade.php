            ...
            <div class="{{ $toc_enabled?'col-md-8':'col-md-10' }} col-sm-12 phone-no-padding">
                <div class="post-detail shadow" id="main-content">
                    ...
                    <div class="post-info-panel">
                        ...
                        <p class="info">
                            <label class="info-title">{{__('web.CREATE_DATE')}}:</label>{{ $post->created_at->format('Y, m, d') }}
                        </p>
                        @if(isset($post->published_at) && $post->published_at)
                            <p class="info">
                                <label class="info-title">{{__('web.EDIT_DATE')}}:</label>{{ $post->published_at->format('Y, m, d') }}
                            </p>
                        @endif
                        <p class="info">
